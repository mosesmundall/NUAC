...
const CLUB = {
  ...
  allowedWeights: ["women", "woman", "u60kg", "u75kg", "u85kg", "open"],
};

const SHEETS_SCOPE = "https://www.googleapis.com/auth/spreadsheets";
const GOOGLE_TOKEN_URL = "https://oauth2.googleapis.com/token";
const SYDNEY_TZ = "Australia/Sydney";
const NUAC_AU_CUTOVER = "2026-08-01";

let cachedGoogleToken = null;
let cachedGoogleTokenExpiresAt = 0;

const trim = (value) => String(value ?? "").trim();
const normKey = (value) => trim(value).toLowerCase().replace(/[^a-z0-9]/g, "");
...
async function getGoogleAccessToken(email, privateKey) {
  ...
  if (!response.ok || !payload.access_token) {
    throw new Error(`Google authentication failed (${response.status}). Check the service-account email/private key.`);
  }
...
async function googleJson(url, accessToken, options = {}) {
  ...
  if (!response.ok) {
    const err = new Error(data?.error?.message || `Google Sheets request failed (${response.status}).`);
...
// Historical NUAC rows before 01/08/2026 were entered as US MM/DD/YYYY.
// Rows from the migration onward are expected as AU DD/MM/YYYY. Keeping this
// parser preserves the existing historical replay while new form submissions
// are stored as real Sheets dates and explicitly formatted DD/MM/YYYY.
function storedDateToIso(value) {
  ...
  const dmyPost = dmy >= NUAC_AU_CUTOVER;
  const mdyPost = mdy >= NUAC_AU_CUTOVER;
...
async function forceAustralianDateFormat(spreadsheetId, sheetId, rowNumber, columnIndex, accessToken) {
  if (!rowNumber) return;
...
export default async (request) => {
  ...
  try {
    ...
    const hidePublicActivity = body.hidePublicActivity === true;
    ...
    if (matchMap.badge < 0) {
      return reply(500, {
        ok: false,
        error: 'Match sheet is missing the required Badge? column. Add a column headed exactly "Badge?" before using Match Entry.',
      });
    }

    const coreIndexes = [matchMap.date, matchMap.winner, matchMap.loser, matchMap.arm, matchMap.badge];
    ...
    const duplicateIndex = existingRows.findIndex((row) => {
      ...
      return existingDateIso === dateIso &&
    ...
    // DATE() is locale-independent and always creates a real Sheets date serial.
    // We then force this specific new cell to DD/MM/YYYY so GViz exports all new
    // NUAC entries consistently, while old historical rows keep their old format.
    row[matchMap.date] = isoToDateFormula(dateIso);
    row[matchMap.winner] = winnerId;
    row[matchMap.loser] = loserId;
    row[matchMap.arm] = arm;
    row[matchMap.badge] = hidePublicActivity ? "FALSE" : "";
    ...
    await forceAustralianDateFormat(
      CLUB.matches.id,
      CLUB.matches.gid,
      writtenRow,
      matchMap.date,
    ...
    return reply(200, {
      ...
      loserId,
      arm,
      hiddenRankingAdjustment: hidePublicActivity,
    });
  } catch (error) {
